...
When interpreting or plotting interactions becomes messy, summarize predicted values. To calculate the predictions, use the `fitted()` function and save them as a new variable.

```{r}
midd_survey$pred_lookrel <- 
fitted(lookrel_genderXyear_model)
```
...
Want a nice table? Use `pander()` with the object you just created.
...
```{r}
midd_survey$pred_newpeople <- fitted(newpeople_raceXclass_model)

new_people_predictions <- midd_survey %>%
     group_by(race, class) %>%
