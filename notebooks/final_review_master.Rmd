...
review <- read.csv("/users/lawrence/desktop/final_review_2019/final_review.csv")
library(tidyverse)
library(pander)
library(stargazer)
```
...
The big thing to do before starting analyses is to code all missing values as NA. Look to the do file or the excel file downloaded from the GSS website to see how missing values are coded for each variable. We can see that for the `age` variable missing values have a code of 99. Create a table of the `age` variable to see if there are missing values:

```{r}
table(review$age)
```

There are 34 observations with a missing value code of 99 for age. Here's how we can make them NAs using `na_if` (which is efficient if there is only one missing value):
...
Here are examples with `ifelse` (which is efficient if there are multiple missing values):
...
Replace missing values for the `hispanic` variable:
