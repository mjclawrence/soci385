...
week9 <- read.csv("https://raw.githubusercontent.com/mjclawrence/soci385/master/data/week_9.csv")
library(tidyverse)
library(pander)
```
...
```{r}
chisq.test(week9$sex, week9$courts)
#qchisq(.95, df = 12)
```

```{r}
chisq.test(week9$born, week9$wrkslf)
```
