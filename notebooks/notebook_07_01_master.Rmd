...
Let's test if the mean value of `ideal_childs` in 2014 differed from 2.41 (the 1984 mean).

What was the mean for `ideal_childs` in 2014?

### REPLACE THIS LINE WITH YOUR CODE

```{r find mean for sample year}
mean(gss_week7$ideal_childs[gss_week7$year==2014])
```
...
R has a built-in function called `t.test()` that will calculate all of these test statistics. With one mean, we have to fill in the value of mu which is the null hypothesis value. If you leave it out, the default is zero.

```{r t test in r}
t.test(gss_week7$ideal_childs[gss_week7$year==2014], mu = 2.41)
```

For another example, let's look at 2010's mean number of children (variable = `childs`). Using `t.test()`, can we reject the null hypothesis that the 2010 mean is the same as 2.04 at the .05 alpha level?

### REPLACE THIS LINE WITH YOUR CODE

```{r t test in r exercise}
t.test(gss_week7$childs[gss_week7$year==2010], mu = 2.04)
```

Can we reject the null hypothesis that the 2010 mean is the same as 1.8 at the .01 alpha level? We can change the default level of .05 to .01 using the conf.level option (which requires the confidence level, so .99 for the .01 alpha level).

### REPLACE THIS LINE WITH YOUR CODE

```{r t test 2 in r exercise}
t.test(gss_week7$childs[gss_week7$year==2010], mu = 1.8,
       conf.level = .99)
```