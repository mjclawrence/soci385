...
Question 1: At the .05 alpha level, can you reject the null hypothesis that the mean number of memberships (`memnum`) is 1.5? Why or why not?
...
Question 2: At the .01 alpha level, can you reject the null hypothesis that the mean number of memberships is 1.5? Why or why not?
...
subset <- mutate(subset, college = ifelse(
      degree=="College Degree" | degree =="Grad/Prof Degree", 
      1, 0))
```
...
```{r}
t.test(week_8_1$agekdbrn[week_8_1$racehisp=="White"],
       week_8_1$agekdbrn[week_8_1$racehisp=="Black"])
```
...
```{r}
t.test(week_8_1$polactions[week_8_1$class=="Lower" & week_8_1$sex=="Male"],
       week_8_1$polactions[week_8_1$class=="Lower" & week_8_1$sex=="Female"])
```
