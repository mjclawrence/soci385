...
ui <- fluidPage(
  ...
  mainPanel( # details for the main panel in the center of the screen
    tabsetPanel( # establishing that we will have tabbed windows in this panel
      ...
      ), # Add a comma here to add another tab to the main panel
      tabPanel( # We want a new tab
        "Summary of Variables", # With this name
        div( # Having these characteristics
          style = "font-size: 20px", 
          verbatimTextOutput("summary") # Displaying this info
        ) # Close the div open parentheses
      ) # Close the tabPanel open parentheses
...
# Add this before closing the server function curly brackets
  
  ### Generate a summary of the dataset
  output$summary <- renderPrint({ # Name the object that will print in the summary tab 
    dataset <- cz %>% # Create a subset of the dataframe...
      select(input$x_variable, input$y_variable) # ...with only these variables 
    
    summary(dataset) # And run a summary of that subset
  }) # Close the output$summary setup
